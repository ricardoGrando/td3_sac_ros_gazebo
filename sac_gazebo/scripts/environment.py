...
class Env():
    ...
    def set_reward(self, done, action):
        ...
        if done and not self.get_goalbox:
            ...
            self.get_goalbox = False
            self.reset()
        # if self.last_distance > self.get_goal_distance() and not done and not self.get_goalbox and abs(self.heading) > 0.05:
        #     reward += abs(action[0])*(1/self.get_goal_distance())*(1/self.heading)
    ...
    def step(self, action):
        ...
        reward = self.set_reward(done, action)
